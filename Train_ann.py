...
dataset=pd.read_csv("Data\\data_5.csv")

#dataset = dataset.sample(136000)
...
dataset.drop("x_1",axis=1,inplace=True)
dataset.drop("x_2",axis=1,inplace=True)
dataset.drop("x_3",axis=1,inplace=True)
dataset.drop("x_4",axis=1,inplace=True)
dataset.drop("x_5",axis=1,inplace=True)
dataset.drop("y_1",axis=1,inplace=True)
dataset.drop("y_2",axis=1,inplace=True)
dataset.drop("y_3",axis=1,inplace=True)
dataset.drop("y_4",axis=1,inplace=True)
dataset.drop("y_5",axis=1,inplace=True)
...
dataset["delta_lon"] = dataset["x_to"] - dataset["x_from"]
dataset["delta_lat"] = dataset["y_to"] - dataset["y_from"]

dataset.drop("TIMESTAMP",axis=1,inplace=True)
tenp_column=dataset.pop('y_to')
dataset.insert(len(dataset.columns),"y_to",tenp_column)
tenp_column=dataset.pop('x_to')
dataset.insert(len(dataset.columns),"x_to",tenp_column)
...
dataset_test['Week'] = dataset_test['Week'].astype('int32')
...
dataset_test["delta_lon"] = dataset_test["x_to"] - dataset_test["x_from"]
dataset_test["delta_lat"] = dataset_test["y_to"] - dataset_test["y_from"]
...
dataset_sb=pd.read_csv("Data\\submission.csv")
dataset_sb
...
mse = tf.keras.losses.MeanSquaredError()
print(mse(y_sb,test_y_pred).numpy())
output = pd.DataFrame({"TRIP_ID":id, "LATITUDE":test_y_pred.T[0],"LONGITUDE":test_y_pred.T[1]})
output.to_csv("Data\\submission_ann_6_novector.csv")